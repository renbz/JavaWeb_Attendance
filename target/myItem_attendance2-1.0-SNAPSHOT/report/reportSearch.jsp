...
<body>
    ...
    <br />
    <ul class="seachform">
        <form action="${pageContext.request.contextPath}/R01_FindReportByPageServlet" method="post">
   <li>
      <label>姓名</label><input name="search_name" type="text" class="scinput" /></li>
    <li>
      ...
      <tbody>
        <tr>
          <td><table class="tablelist">
              ...
              <tbody>
              <c:forEach items="${pb.list}" var="rps" varStatus="s">
                  <tr>
                      <td><input name="uid" type="checkbox" value="${rp.id}"/></td>
                      <td>${rps.account}</td>
                      <td>${rps.name}</td>
                      <td>${rps.report_date}</td>
                      <td>${rps.work_process}</td>
                      <td>${rps.work_content}</td>
                      <td>${rps.problem}</td>
                      <td> </td>

                          <%--修改按钮 --%>
                      <td><span><a href="${pageContext.request.contextPath}/R03_EchoReportDataServlet?id=${rps.report_id}"
                                   class="tablelink"><img src="../images/user_edit.png"/>修改</a>
                        <%--删除按钮同理 --%>
                        <a href="${pageContext.request.contextPath}/R02_ReportDeleteServlet?id=${rps.report_id}" class="tablelink" onclick="confirm('确定要删除吗？')"> <img
        ...
        <ul class="paginList">
            ...
            <c:forEach begin="1" end="${pb.totalPage}" var="i">
                <c:if test="${pb.currentPage==i && (i <=5 || i >=pb.totalPage-4)}">
                    <li class="paginItem current"><a href="${pageContext.request.contextPath}/R01_FindReportByPageServlet?currentPage=${i}&rows=5">${i}</a></li>
                </c:if>
                <c:if test="${pb.currentPage!=i && (i <=5 || i >=pb.totalPage-4)}">
                    <li class="paginItem"><a href="${pageContext.request.contextPath}/R01_FindReportByPageServlet?currentPage=${i}&rows=5">${i}</a></li>
                </c:if>
                <c:if test="${i == 6 }">
